...
# Section 3
# typical access.log format 1 - may not be accurate. check time based on `| head -n 2` output
...
# Section 4
# typical access.log format 2 - may not be accurate. check time based on `| head -n 2` output
...
# PHP
# Section 5
# typical php.access.log format 1 - may not be accurate. check time based on `| head -n 2` output
...
# Section 6
# typical php.access.log format 2 - may not be accurate. check time based on `| head -n 2` output
...
grep -e '^pm.max_children' /etc/platform/*/php-fpm.conf # max children available
...
## to check queries on the current node that you are on 
# mysql -u $DB_USER -h $DB_HOST --password="$DB_PASS" -P $DB_PORT_LOCAL_NODE $DB_PATH -e 'show processlist;' | grep -v Sleep | wc -l
# mysql -u $DB_USER -h $DB_HOST --password="$DB_PASS" -P $DB_PORT_LOCAL_NODE $DB_PATH -e 'show processlist;' | grep -v Sleep
# mysql -u $DB_USER -h $DB_HOST --password="$DB_PASS" -P $DB_PORT_LOCAL_NODE $DB_PATH -e 'show full processlist;' | grep -v Sleep
# mysql -u $DB_USER -h $DB_HOST --password="$DB_PASS" -P $DB_PORT_LOCAL_NODE $DB_PATH -e 'show processlist;' | grep Sleep | wc -l
